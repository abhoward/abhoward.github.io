...
save_file_path = "..\\..\\data\\Patches\\7.29d\\{}"
dfs_to_convert = {}
jsons_to_upload = {}
...
# time at which the patch was added
start_time = 1621882501
...
matches = df[df['start_time'] >= start_time]
...
# Clearing excluded matches text file
open(save_file_path.format('excluded_matches.txt'), 'w').close()
...
for match_id in matches['match_id'].tolist():
    try:
        ...
        with open(save_file_path.format('excluded_matches.txt'), 'a') as f:
            f.write(str(match_id) + '\n')
            print('Match {} excluded.'.format(match_id))
...
for team in sorted(team_hero_wins['team'].astype(str).unique()):
